    ...
    \column{0.42}
      \block{Introduction}{
		\innerblock{Motivation}{
		  \begin{minipage}[t]{.63\colwidth}
			\begin{itemize}
			  \item {\bf Aim:} gain more {\bf precise understanding} of {\bf bonding} between {\bf adsorbates} and {\bf metals}
                            \newline $\Rightarrow$ Fundamentally important for {\bf Catalysis}
			  \item {\bf Why femtosecond(fs)-lasers?} 
			  \begin{itemize}
				\item {\bf cause} highly non-equilibrium {\bf 2-T-states} \newline $\Rightarrow$ {\bf different} processes {\bf than} from {\bf heating}
				\item further {\bf tool besides STM} and {\bf scattering} 
...
% 				\includegraphics[width=.18\colwidth]{abb/nanofemto.png}
				%$\,$\\[.6\baselineskip]
				\includegraphics[width=.22\colwidth]{abb/CO_CO2.png}
				\begin{small}example for femtochemistry                                                  \end{small}
			  \end{flushright}
			\end{minipage}
		  \end{adjustbox}
		  \vspace*{-2.2\baselineskip}
                        \begin{itemize}
                          \item {\bf Why} study {\bf CO/Ru(001)} or {\bf CO/Cu(100)}? 
                          \begin{itemize}
                            ...
                            \item moreover, significant {\bf progress} in {\bf theory} for both systems\cite{fuechsel:14}\cite{marqu:10}\newline $\Rightarrow$ Very accurate ({\bf ab-initio} based) {\bf 6-dim. potentials} available
                            \item also: {\bf theory} has to {\bf start small!} Anyhow, in future {\bf possible} to {\bf describe bigger systems with} the herein {\bf presented methods}
                          \end{itemize}                   
                        \end{itemize}
% 		  \begin{itemize}
...
% 					ion lattice plus quasi-free electron gas 
			  \item {\bf laser} is {\bf absorbed by metal electrons} only
			  \item produced {\bf electron-hole pairs} thermalize quickly  
					$\Rightarrow$ {\bf ``hot''} Fermi-Dirac-distribution
% 				  \item Fermi-Dirac schon hier?
						% \vbox syntax? (for the graphic)
			  \item {\bf electrons transfer} part of {\bf energy} to ion {\bf lattice}, via \raisebox{-.18\baselineskip}{\includegraphics[height=.8\baselineskip]{abb/one.png}}
			  \hspace{-0.4cm} \textbf{electron-phonon coupling}%\newline (phonons = lattice vibrations; quasi-particles)
                          \item {\bf equilibration} complete {\bf within ps}-timescale			
			  \item[$\Rightarrow$] {\bf Thus}, with {\bf fs-lasers two temperatures}:
			  \begin{itemize}
...
% 					\item Two-Temperature Model (TTM)\cite{anisimov:74}
			  \end{itemize}
			  \item {\bf both couple to adsorbed molecule}
                          \item described by {\bf Two-Temperature Model} \cite{anisimov:74} 
			\end{itemize}
		  \end{minipage}
		  \begin{adjustbox}{valign=t}
...
% 				\includegraphics[width=.18\colwidth]{abb/elgas.png}$\,$\\[.2\baselineskip]
				\includegraphics[width=.24\colwidth]{abb/fermi.png}\vspace*{-.3\baselineskip}\\
	  ...
	  \block{Models and Methods}{
		\innerblock{Six-dimensional Potential Energy Surface (6D PES)\cite{fuechsel:14}}{
		  \begin{minipage}[t]{.66\colwidth}
			\begin{itemize}
			  \item Basis for dynamics: {\bf precomputed PES} from DFT (rPBE + D2) 
			\end{itemize}
			\begin{adjustbox}{valign=t}
...
% 				  \vspace*{.5\baselineskip}
			...
			\begin{minipage}[t]{.46\colwidth}
			  \begin{itemize}
				\item {\bf all 6 dimensions} of the adsorbate
				\item {\bf analytical} PES and gradients $\Rightarrow$ {\bf very fast} % (once constructed)
				\item[$\Rightarrow$] {\bf number}, length of {\bf trajectories} can be {\bf large} 
				\item {\bf downsides}: -- surface frozen $\Rightarrow$ {\bf no phonons} 
				\newline \hspace*{5.5cm} -- had to be constructed first
			  \end{itemize}
			\end{minipage}
		  \end{minipage}
		  \begin{adjustbox}{valign=t}
			\begin{minipage}[t]{.35\colwidth}
			  \hspace*{-.01\colwidth}\includegraphics[width=.28\colwidth]{abb/fig_1b.pdf}\vspace*{-0.7\baselineskip}\\
			  \hspace*{.05\colwidth}\begin{small}Potential for CO/Ru(001)                                          \end{small}
			\end{minipage}
		  \end{adjustbox}  
		}
		\vspace{.5cm}
...
% 			    \item ``Local Density'' $\Rightarrow$ homogenous free electron gas (FEG)
			...
			\end{itemize}
			  ...
			  \begin{itemize}
				\item justified by {\bf 2. fluctuation dissipation theorem}\cite{kubo:66} (relating friction and thermal movement) 
			  \end{itemize}  
		  \end{itemize}
