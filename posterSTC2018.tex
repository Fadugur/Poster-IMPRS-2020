...
% 		  \end{minipage}
		...
		\begin{adjustbox}{valign=t}
                    \begin{minipage}[t]{.22\colwidth}
                          \includegraphics[width=.24\colwidth]{abb/Part1SurfScheme.png}
%                               \includegraphics[width=.3\colwidth]{abb/2TM_equs.png}
		...
		\begin{adjustbox}{valign=t}
                  \begin{minipage}[t]{.47\colwidth}
                        \vspace*{.3cm}
			\begin{itemize}
			  \item simulates {\bf interaction} of {\bf electrons} with {\bf phonons} and {\bf laser} 
			   $\Rightarrow$ {\bf gives $\boldsymbol{T}_\mathbf{el}$ and $\boldsymbol{T}_\mathbf{ph}$ as $\boldsymbol{f(z,t)}$} 
			\end{itemize}			  
			\begin{center}
			  \includegraphics[width=.42\colwidth]{abb/2TM_equs.png}
			\end{center}			
                  \end{minipage}
			\end{adjustbox}
...
%                             \item[$\Rightarrow$]
    ...
    %                         \begin{minipage}[t]{.5\colwidth}
                                  ...
                                  \item elec. heat conductivity $\kappa$                       
                                  \item elec.-phonon coupling $g$
                                  \item laser source term S(z,t) 
    %                         \end{minipage}
                          \end{itemize}                           
    %                       }
%                       \end{itemize}                           
		...
		\innerblock{Electronic Friction: Langevin Dynamics\cite{headgordon:95} and Local~Density~Friction~Approximation~(LDFA)\cite{juaristi:08}}{
		  \begin{adjustbox}{valign=t}
			\begin{minipage}[t]{.26\colwidth}
			  \includegraphics[width=.24\colwidth]{abb/Part2SurfScheme.png}
% 				\includegraphics[width=.3\colwidth]{abb/2TM_equs.png}
			\end{minipage}
		  \end{adjustbox}
		  \begin{minipage}[t]{.7\colwidth}
			\begin{itemize}
			  \item the {\bf Langevin equation} of motion, a {\bf stochastical} differential equation:%\\[-.9\baselineskip]
% 				\vspace{.2\baselineskip}
% 				\item describes interaction of  electron-hole-pairs with molecule
			\end{itemize}
					  
			\includegraphics[width=.67\colwidth]{abb/Langevin_eq.png}
		  \end{minipage}
% 		  \vspace*{-.1\baselineskip}
		  \begin{itemize}
			\item describes {\bf movement of CO} and {\bf interaction with electron-hole pairs} (friction and excitation)
			\item {\bf Local Density Friction Approx.} (LDFA): \textit{ab-initio} model that gives {\bf friction coefficients} $\boldsymbol{\eta}_{\mathbf{el},\boldsymbol{k}}$
			\begin{itemize}
...
% 			\end{adjustbox}
		...
		\innerblock{Inclusion of Phonons: Generalized Langevin Oscillator(GLO)-model\cite{busnengo:05}}{
		  \begin{adjustbox}{valign=t}
			\begin{minipage}[t]{.24\colwidth}
			  \includegraphics[width=.24\colwidth]{abb/Part3SurfScheme.png}
% 				\includegraphics[width=.3\colwidth]{abb/2TM_equs.png}
			\end{minipage}
		  \end{adjustbox}
		  \begin{minipage}[t]{.7\colwidth}
			\begin{itemize}
			  \item influence of {\bf phonons} effectively {\bf modeled} ({\bf augments frozen surface})
			  \item {\bf entire surface} understood as {\bf 3D oscillator} (coords. $\underline{r}_s$, mass of one atom)
			  ...
			  \begin{itemize}
				\item ghost oscillator is subject to {\bf friction} $\boldsymbol{\eta}_\mathbf{ph}$  and {\bf random forces} $\underline{\boldsymbol{R}}_\mathrm{ph}(T_\mathbf{ph})$
			  \end{itemize}
			\end{itemize}
% 			  {\Large
% 			  \begin{equation*}
...
%     \vspace*{1\baselineskip}
  \begin{columns}
  \column{.72}
  \block[titleoffsety=3cm,bodyoffsety=3cm,titleoffsetx=-.24\colwidth,titlewidthscale=.511,]{Results}{
	\begin{adjustbox}{valign=t}
...
% 		\end{itemize}
		\begin{adjustbox}{valign=t}
		  \begin{minipage}[t]{.21\colwidth}
                        \includegraphics[width=.22\colwidth]{abb/fig_9.eps}\\
                        \vspace*{-1.\baselineskip}
                        \includegraphics[width=.22\colwidth]{abb/fig_8.png}\\
                        \begin{small}
                          \hspace*{2.5cm} Ru (with GLO) 
                        \end{small}
...
% 		    \item desorption mainly during first 50 ps
		    \item {\bf fluence dependence} of desorption yield close to experiment (Ru)
		    \item GLO affects desorption differently
% 		    \item no ``precursor state'' as suggested by \cite{nilsson:13}
% 		    \item no barrier in PMF
		  \end{itemize}
		  \hspace*{.5cm}\includegraphics[width=.25\colwidth]{abb/Z-Histogramme.png}\\
		        \begin{small}
                          \hspace*{2.5cm} Cu (top with GLO, bottom w/o)
                        \end{small}
		\end{minipage}



% 		\includegraphics[width=.19\colwidth]{abb/}		
	  }	 
	  \vspace{.3cm}
	        \innerblock{CO-stretch Vibration (Data for Cu)}{
%           \begin{adjustbox}{valign=t}          
...
%           \end{adjustbox}
          \begin{adjustbox}{valign=t}          
            \begin{minipage}[t]{.16\colwidth}
              \includegraphics[height=5.5\baselineskip]{abb/Inoue.png}\\
              \begin{small}
                \hspace*{2.cm} SFG-experiment\cite{inoue:16}
              \end{small}
            \end{minipage}
          \end{adjustbox}   
          \begin{adjustbox}{valign=t}          
            \begin{minipage}[t]{.3\colwidth}
              \includegraphics[height=4\baselineskip]{abb/Durchschnitt.pdf} \hspace*{.5cm}\includegraphics[height=4.5\baselineskip]{abb/Fourier.pdf} \\
                            \begin{small}
                \hspace*{5.cm} from our dynamics
              \end{small}
            \end{minipage}
	  ...
	  \begin{minipage}[t]{.46\colwidth}
		  ...
		  \begin{adjustbox}{valign=t}
			\begin{minipage}[t]{.20\colwidth}	  	
			  \includegraphics[width=.20\colwidth]{abb/Trajektorie_gross.png}\\

			\end{minipage}
		  \end{adjustbox} 
		  \begin{minipage}[t]{.22\colwidth}
			\begin{itemize}
			  \item {\bf typical trajectory} (Ru): {\bf vibration} around top and 
			   {\bf hops between top sites}  
			  \item {\bf increase} in $\boldsymbol{\theta}${\bf-angle} and {\bf decrease of Z}
					when CO moves {\bf away} from {\bf top}
               
			\end{itemize}
		  \end{minipage}

		                          \includegraphics[width=.16\colwidth]{abb/grid640_theta.png} \hspace{1.5cm}
                          \includegraphics[width=.16\colwidth]{abb/grid640_z.png}\\



		  \vspace{-1.5\baselineskip}
		  \begin{itemize}
		                              \item {\bf dynamical trapping effect} at high energy sites   
% % 		    \item[$\Rightarrow$] possible alternative explanation to ``precursor state'' \cite{nilsson:13}
		  ...
		  \begin{small}
                    \hspace*{1cm} CO XY-``heat map'' on Ru (timescale several ps), molecules favor hcp site after pulse 
                  \end{small}\\
                  \vspace*{.3\baselineskip}\\
                  \includegraphics[width=.445\colwidth]{abb/Movie_COCu.png}\\
                  \begin{small}
                     CO XY-``heat map'' on Cu (timescale below one ps), molecules favor bridge site after pulse              
                  \end{small}\\
  ...
  \block{}{
	...
	\begin{itemize}
	  \item {\bf system: CO} on {\bf Ru(001) or Cu(100)} 
	  \item {\bf full-dimensional (6D)} simulation of \newline dynamics driven by {\bf fs-laser irradiation} 
	  \item {\bf first principles}, no ``free'' parameters
	  \item accounting for {\bf1) electronic friction} and excitation by hot electron-hole pairs (via LDFA and the Langevin approach) and {\bf 2) substrate motion} (via GLO)
	  \item detailed time- and space-resolved insights
% 	  \item no physisorbed state, molecules desorp directly 
%         time for more COCu gang stuff! =)
	\end{itemize}
	}
	\vspace{.5cm}
	\innerblock{Outlook}{
	\begin{itemize}
% 	  \item 
	  \item better description of electronic friction: $\boldsymbol{\eta}_\mathbf{el}$ {\bf as} $\boldsymbol{f}(\boldsymbol{T}_\mathbf{el})$ and going {\bf beyond LDFA}
	  $\Rightarrow$ Long term goal: {\bf friction tensors}\cite{maurer:16}  
	  \item further {\bf2TM improvements}, consider {\bf non-equilibrium~lattice~(NLM)}\cite{waldecker:16} 
	  
	  \item directly model {\bf SFG influence} by explicitly including {\bf excitation} from {\bf IR-pulse}  
	  
	  \item {\bf larger super cells} for other {\bf  coverages} and {\bf intermolecular  interaction} 	  
	  \item simulate {\bf more complex systems}: \newline hydrocarbons; coadsorbate CO + H or O  
% 	  \item include interaction   
	\end{itemize}
...
% 		  M. Dell'Angela, T. Anniyev, M. Beye, R. Coffee, A. F\"ohlisch \emph{et al.}, 
		...
		\bibitem{juaristi:08}
		J.I. Juaristi, M. Alducin, R. Díez Muiño \emph{et al.}, 
...
% 		J.C. Tully, \emph{J.Chem.Phys.} \textbf{73}, 1975 (1980).
		...
		\bibitem{busnengo:05}
		H.F. Busnengo, M.A. Di Césare, W. Dong \emph{et al.},
		\emph{Phys.Rev.B} \textbf{72}, 125411 (2005).
		
		\bibitem{maurer:16}
                R. J. Maurer, M. Askerka, V. S. Batista, J. C. Tully
                \emph{Phys.Rev.B} \textbf{94}, 115432 (2016).
		
                \bibitem{waldecker:16}
                L. Waldecker, R. Bertoni, R. Ernstorfer, J. Vorberger
                \emph{Phys.Rev.X} \textbf{6}, 021003 (2016).
